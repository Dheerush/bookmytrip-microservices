...
const makeFareCategories = (base: { sleeper: number; ac3Tier: number; ac2Tier: number; ac1st: number; }) => {
...
const raw = [
  { trainNumber: '12951', name: 'Mumbai Rajdhani Express', from: 'New Delhi', fromCode: 'NDLS', to: 'Mumbai Central', toCode: 'BCT', departureTime: '16:55', arrivalTime: '08:35', duration: '15h 40m', daysOfWeek: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], pnr: 'PNR284719', fare: { general: 450, sleeper: 780, ac3Tier: 1890, ac2Tier: 2780, ac1st: 4720 }, seatsAvailable: { general: 120, sleeper: 45, ac3Tier: 32, ac2Tier: 18, ac1st: 6 }, type: 'Rajdhani', stops: 5, rating: 4.6 },
  { trainNumber: '12301', name: 'Howrah Rajdhani Express', from: 'New Delhi', fromCode: 'NDLS', to: 'Howrah Jn', toCode: 'HWH', departureTime: '16:50', arrivalTime: '09:55', duration: '17h 05m', daysOfWeek: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], pnr: 'PNR381045', fare: { general: 520, sleeper: 890, ac3Tier: 2100, ac2Tier: 3100, ac1st: 5200 }, seatsAvailable: { general: 90, sleeper: 30, ac3Tier: 22, ac2Tier: 14, ac1st: 4 }, type: 'Rajdhani', stops: 4, rating: 4.7 },
  { trainNumber: '12002', name: 'New Delhi Shatabdi Express', from: 'New Delhi', fromCode: 'NDLS', to: 'Bhopal Jn', toCode: 'BPL', departureTime: '06:15', arrivalTime: '14:10', duration: '7h 55m', daysOfWeek: ['Mon','Tue','Wed','Thu','Fri','Sat'], pnr: 'PNR492830', fare: { general: 350, sleeper: 650, ac3Tier: 1350, ac2Tier: 1900, ac1st: 3200 }, seatsAvailable: { general: 200, sleeper: 60, ac3Tier: 40, ac2Tier: 26, ac1st: 8 }, type: 'Shatabdi', stops: 3, rating: 4.5 },
  { trainNumber: '12259', name: 'Sealdah Duronto Express', from: 'New Delhi', fromCode: 'NDLS', to: 'Sealdah', toCode: 'SDAH', departureTime: '20:10', arrivalTime: '11:30', duration: '15h 20m', daysOfWeek: ['Mon','Thu','Sat'], pnr: 'PNR571924', fare: { general: 480, sleeper: 820, ac3Tier: 1950, ac2Tier: 2900, ac1st: 4900 }, seatsAvailable: { general: 80, sleeper: 28, ac3Tier: 18, ac2Tier: 10, ac1st: 4 }, type: 'Duronto', stops: 0, rating: 4.4 },
  { trainNumber: '12627', name: 'Karnataka Express', from: 'New Delhi', fromCode: 'NDLS', to: 'Bengaluru', toCode: 'SBC', departureTime: '21:20', arrivalTime: '06:40', duration: '33h 20m', daysOfWeek: ['Mon','Tue','Wed','Thu','Fri','Sat','Sun'], pnr: 'PNR693815', fare: { general: 620, sleeper: 950, ac3Tier: 2350, ac2Tier: 3400, ac1st: 5800 }, seatsAvailable: { general: 150, sleeper: 55, ac3Tier: 38, ac2Tier: 20, ac1st: 6 }, type: 'Superfast', stops: 12, rating: 4.3 },
  { trainNumber: '12431', name: 'Thiruvananthapuram Rajdhani', from: 'New Delhi', fromCode: 'NDLS', to: 'Trivandrum Central', toCode: 'TVC', departureTime: '11:25', arrivalTime: '05:15', duration: '41h 50m', daysOfWeek: ['Wed','Sun'], pnr: 'PNR719283', fare: { general: 720, sleeper: 1100, ac3Tier: 2700, ac2Tier: 3900, ac1st: 6500 }, seatsAvailable: { general: 60, sleeper: 22, ac3Tier: 15, ac2Tier: 8, ac1st: 3 }, type: 'Rajdhani', stops: 8, rating: 4.5 },
...
const seedData = raw.map((train) => ({
  ...train,
  fareCategories: makeFareCategories(train.fare),
  isActive: true,
}));
...
const seed = async () => {
  ...
  if (count > 0) {
    logger.info(`Seed skipped - ${count} trains already exist`);
    await mongoose.disconnect();
    return;
  }

  const inserted = await Train.insertMany(seedData);
  logger.info(`Seeded ${inserted.length} trains`);
  await mongoose.disconnect();
};
